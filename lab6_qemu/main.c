...
#define LOAD_ADDR_PHYS 0x84000000UL
#define CPIO_LOAD_ADDR_PHYS 0x88000000UL
#define NEW_FDT_ADDR_PHYS 0x8c000000UL
...
#define RELOC_ADDR 0x00200000UL
            ...
            if (i < MMIO_MAP_GIB)
            {
                ...
                for (int k = 0; k < PT_ENTRIES; k++)
                {
                    kernel_mmio_pte[i][j][k] =
                        MAKE_PTE(pmd_base + (unsigned long)k * PAGE_SIZE,
                                 PROT_MMIO);
                }
            }
            else
