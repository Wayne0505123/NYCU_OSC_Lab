...
#define RV2_FB_BASE 0x7f700000UL
#define RV2_FB_SIZE (1920UL * 1080UL * 4UL)
...
static int is_kernel_va(unsigned long addr) {
    return addr >= PAGE_OFFSET;
}

static unsigned long phys_to_virt_addr(unsigned long pa) {
    return pa + PAGE_OFFSET;
}

static unsigned long virt_to_phys_addr(unsigned long va) {
...
static unsigned long page_phys_addr(struct page *p) {
    return mem_base + page_idx(p) * PAGE_SIZE;
}

static unsigned long page_addr(struct page *p) {
    return phys_to_virt_addr(page_phys_addr(p));
}

static struct page *addr_to_page(unsigned long addr) {
    unsigned long phys = virt_to_phys_addr(addr);
    unsigned long idx = (phys - mem_base) / PAGE_SIZE;
    return &mem_map[idx];
}
...
static void *startup_alloc(unsigned long size, unsigned long align) {
    ...
    while (p + size <= mem_base + mem_size) {
        if (!overlap_reserved(p, p + size)) {
            ...
            return (void *)phys_to_virt_addr(p);
        }
...
static struct page *page_from_alloc_ptr(void *ptr) {
    unsigned long addr;
    unsigned long phys;
    ...
    phys = virt_to_phys_addr(addr);
    if (phys < mem_base || phys >= mem_base + mem_size)
        return 0;

    page_base = addr & ~(PAGE_SIZE - 1);
    return addr_to_page(page_base);
}

void retain_page(void *ptr) {
    struct page *p = page_from_alloc_ptr(ptr);
...
void mm_init_advanced(const void *fdt,
	...
	add_reserved(virt_to_phys_addr((unsigned long)fdt),
	             bswap32_mm(hdr->totalsize));
	...
	unsigned long kernel_end = virt_to_phys_addr((unsigned long)_end);
	...
	if (initrd_start_addr && initrd_end_addr > initrd_start_addr) {
		add_reserved(virt_to_phys_addr(initrd_start_addr),
		             initrd_end_addr - initrd_start_addr);
	}

	/* RV2 framebuffer is initialized by U-Boot and may not be listed in DTB. */
	add_reserved(RV2_FB_BASE, RV2_FB_SIZE);

	startup_ptr = align_up_ul(virt_to_phys_addr((unsigned long)_end),
	                          PAGE_SIZE);
