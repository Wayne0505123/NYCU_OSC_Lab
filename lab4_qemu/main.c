...
extern unsigned char uart_read_reg(int off);
extern void uart_write_reg(int off, unsigned char val);
...
#define LOAD_ADDR   ((unsigned char *)0x80200000UL)
#define RELOC_ADDR  0x80400000UL
...
static inline unsigned long irq_save(void) {
    ...
    /*
     * csrrci:
     * 1. 先把目前 sstatus 讀到 flags
     * 2. 再把 sstatus.SIE bit 清掉，也就是關 interrupt
     *
     * SIE bit = bit 1 = 0x2
     */
...
static inline void irq_restore(unsigned long flags) {
    /*
     * 如果進入 run_tasks() 前 SIE 是開的，離開時就重新開啟。
     * 如果原本是關的，離開時就保持關閉。
     */
...
static inline void set_stvec(void) {
...
static void run_tasks(void) {
    ...
    while (1) {
        ...
        /*
         * 執行 task callback 時暫時開 interrupt，
         * 讓更高 priority 的 interrupt / task 有機會進來。
         */
        local_irq_enable();
        cb(arg);

        /*
         * callback 結束後先關 interrupt，
         * 避免操作 task queue 狀態時被中斷插入。
         */
        local_irq_disable();
    ...
    /*
     * 離開 run_tasks() 前恢復進入前的 interrupt 狀態。
     */
    irq_restore(flags);
}
...
static void uart_interrupt_init(void) {
    uart_write_reg(UART_MCR, uart_read_reg(UART_MCR) | UART_MCR_OUT2);
...
static int exec_user_program(const char *filename) {
     ...
     * SPIE = 1: interrupts enabled after entering U-mode.
     */
    sstatus &= ~SSTATUS_SPP;
...
void do_trap(struct pt_regs *regs) {
    ...
    if (scause == SCAUSE_U_ECALL) {
    	...
    	if (user_mode_running && user_return_addr) {
            ...
            /*
             * 第 5 次 U-mode ecall 後，把它視為 user program exit，
             * 回到 kernel shell。
             */
            ...
            /*
             * 回 kernel label 時不能用 user sp，
             * 要恢復成 kernel stack。
             */
            	regs->sp = (unsigned long)regs + sizeof(struct pt_regs);
        	} else {
            /*
             * 前 4 次 ecall：跳過 ecall 指令，回到 U-mode 繼續執行。
             */
            	regs->sepc += 4;
        	}	
    	} else {
...
void start_kernel(const void *fdt) {
    ...
    /*
     * Very important:
     * After relocation, stvec must point to relocated handle_exception.
     */
    set_stvec_relocated();
